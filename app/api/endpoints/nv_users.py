...
async def get_my_profile(
    ...
    """
    Récupérer mon profil complet avec portfolio
    """
    try:
        ...
        # ✅ CORRECTION : Formatter le portfolio en List<String> comme les autres endpoints
        portfolio_urls = [item.file_path for item in portfolio_query]
        
        print(f"📸 Portfolio chargé: {len(portfolio_urls)} items")
        print(f"📸 URLs: {portfolio_urls}")
        ...
        profile_data = {
            ...
            "country": current_user.country or "Côte d'Ivoire",
            ...
            "portfolio": portfolio_urls  # ✅ List<String> au lieu de List<Map>
        }
...
async def upload_profile_picture(
    ...
    try:
        ...
        # Maintenant mettre à jour la base de données avec le vrai chemin
        user_service = UserService(db)
        result = user_service.update_profile_picture(
            current_user.id, 
            upload_result["file_url"]  # URL réelle du fichier sauvegardé
        )
        ...
        print(f"✅ Photo de profil sauvegardée: {upload_result['file_path']}")
        ...
        print(f"❌ Erreur upload profile: {e}")
        raise HTTPException(
...
async def upload_cover_picture(
    ...
    # Mêmes validations que la photo de profil
    allowed_types = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif']
    if file.content_type not in allowed_types:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    try:
        ...
        print(f"✅ Photo de couverture sauvegardée: {upload_result['file_path']}")
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Erreur lors de l'upload de la photo de couverture"
        )
